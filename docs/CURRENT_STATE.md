...
- 修复击败小罗后处于 4 秒桑巴冲锋期间掉出关卡时，坠落被无敌状态忽略、角色离开画面但仍持续自动前冲的问题。
- 桑巴无敌继续抵挡敌人和足球攻击，但不再抵挡坠落；坠落会正常扣血并回到检查点。
- 坠落结算时立即清除冲锋计时和庆祝状态，避免复活后继续自动冲锋或人物不可见。
...
## 2026-08-11 游戏内世俱杯造型微调

- 终局粉色 MSN 合捧的世俱杯缩小并下移，减少对内马尔身体的遮挡。
- 将厚重金币式圆盘改为更轻的双层轨道圆环和小型中心星章，删除放射状硬线条。
- 新增连接圆盘与底座的双臂支架，并把底座改为三层结构及 `CWC` 铭牌，使其更像完整奖杯。
- “粉色MSN合捧世俱杯”状态文案改为在浅色提示条内严格垂直居中，不再贴近下边框。
