(() => {
  ...
  function applyFootballerReward(type) {
    ...
    if (type === "messi") {
      ...
      showFootballReward("SIU！", "C罗完成招牌庆祝：获得 10 点积分与宝藏");
  ...
  function drawEnemyFigurine(enemy, time) {
    ...
    if (taunting) {
      ...
      else if (taunt.style === "meditate") { offsetY = 2 + Math.sin(elapsed * 7) * 1.2; scaleY = .9; scaleX = 1.03; }
      else if (taunt.style === "arms") { scaleX = 1.025; offsetY = Math.sin(elapsed * 8) * 1.2; }
      else if (taunt.style === "point") { offsetX = enemy.tauntFacing * progress * 10; offsetY = progress * 3; rotation = enemy.tauntFacing * -.035; scaleY = .95; }
      else if (taunt.style === "dance") { offsetX = Math.sin(elapsed * 13) * 7; rotation = Math.sin(elapsed * 13) * .12; }
      else if (taunt.style === "spread") { offsetY = -3 - Math.sin(progress * Math.PI) * 3; scaleX = 1.03; }
      else if (taunt.style === "slide") { offsetX = enemy.tauntFacing * progress * 11; offsetY = progress * 3; rotation = enemy.tauntFacing * -.025; scaleY = .95; }
    }
  ...
  function drawPlayerFigurine(time) {
    ...
    const trophyWon = hasChampionsLeagueTrophy();
    if (trophyWon) drawChampionsLeagueTrophy(centerX, bottomY - displayHeight * .31, time);
    ctx.save();
    const teamLabel = trophyWon ? "🏆 巴萨MSN合捧欧冠大耳朵杯" : (msnStage === 3 ? "🔵🔴 巴萨MSN 3/3" : (msnStage === 2 ? "MSN 2/3 · 梅西加入" : "内马尔 · 10"));
    const labelWidth = trophyWon ? 174 : (msnStage === 1 ? 78 : 124);
    drawRoundedRect(centerX - labelWidth / 2, bottomY - displayHeight - 20, labelWidth, 20, 8, trophyWon ? "rgba(233,242,255,.97)" : "rgba(255,255,255,.97)", "#172133", 2);
