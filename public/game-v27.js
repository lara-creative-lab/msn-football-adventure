(() => {
  ...
  const NEYMAR_CELEBRATIONS = {
    ...
    vinicius: { label: "桑巴舞步", detail: "暗影彩蛋", style: "dance", duration: 1.85, color: "#e4ad21" },
  ...
  if (bossRewardUnlocked) ownedSkins.add("legend");
  if (!ownedSkins.has(equippedSkin)) equippedSkin = bossRewardUnlocked ? "legend" : (captainUnlocked ? "captain" : (outfitUnlocked ? "starlight" : "default"));
  let shopOpen = false;
  let bestScore = 0;
  ...
  function update(dt) {
    ...
    for (const enemy of enemies) {
      ...
      if (enemy.shotCooldown <= 0 && Math.abs(playerDistance) < 900 && Math.abs(player.y - enemy.y) < 150) {
        ...
        const bossShots = enemy.isBoss && enemy.phase === 2 ? [-235, -155] : [enemy.isBoss ? -205 : -175];
        for (const launchY of bossShots) {
          const size = enemy.isBoss ? 22 : 16;
          footballs.push({ owner: "enemy", x: enemy.x + enemy.w / 2 - size / 2, y: enemy.y + enemy.h * .34, w: size, h: size, vx: direction * (enemy.isBoss ? 410 : 285 + currentLevel * 9), vy: launchY, rotation: 0, bounces: 0, life: 3.2, damage: 1, dead: false });
        }
        enemy.shotCooldown = enemy.isBoss ? (enemy.phase === 2 ? .82 : 1.3) : Math.max(1.05, 2.55 - currentLevel * .08) + ((enemy.x | 0) % 5) * .08;
        beep(enemy.isBoss ? 112 : 165, enemy.isBoss ? .09 : .05, "triangle", enemy.isBoss ? .03 : .018);
      }
  ...
  function defeatFinalBoss(enemy) {
    ...
    rewardDescription.textContent = firstClear
      ? "获得1000宝藏、永久冠军星章与终极冠军战袍；终点旗门已解锁"
      : "获得250宝藏巡回奖励；终点旗门已解锁";
    rewardToast.classList.remove("hidden");
  ...
  function applyFootballerReward(type) {
    ...
    if (type === "messi") {
      ...
      showFootballReward("⚫ 维尼修斯暗影彩蛋！", "桑巴庆祝表情弹出，内马尔变为暗影公仔；MSN巴萨球衣状态下也会生效");
    } else {
  ...
  function updateMsnStage(score) {
    ...
    if (msnStage === 2) {
      ...
      rewardDescription.textContent = "1000分 + 6枚城市印章达成：换上巴萨战袍，继续收集MSN冠军星章";
    }
    rewardToast.classList.remove("hidden");
  ...
  function finish(won) {
    ...
    const rewardText = trophyWon
      ? "十城齐聚、终极Boss落败，巴萨MSN大耳朵杯荣耀仪式完成！"
      : (msnStage === 3
        ? `MSN冠军星章 ${championEmblemCount()}/3；集齐10城并击败终极Boss后举行巴萨MSN捧杯仪式`
        : (msnStage === 2
          ? `下一阶段：累计1000分并收集6城印章，苏亚雷斯加入`
          : `下一阶段：累计500分并收集3城印章，梅西加入`));
    messageKicker.textContent = won ? (trophyWon ? "终极Boss已击破！" : "太棒了！") : "别灰心";
    messageTitle.textContent = won ? (trophyWon ? "MSN十城冠军之旅完成！" : `第 ${currentLevel + 1} 关完成`) : "冒险暂停";
  ...
  function drawHud() {
    ...
    const trophyStatus = boss
      ...
      : (currentLevel === LEVELS.length - 1 && finalBossDefeated
        ? "✓ Boss已击破"
        : (trophyWon ? "🏆 MSN终极荣耀" : (msnStage === 3 ? `✦ 冠军星章 ${championEmblemCount()}/3` : `📍 城市 ${stampCount}/10`)));
