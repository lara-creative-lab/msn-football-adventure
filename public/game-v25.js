(() => {
  ...
  const NEYMAR_CELEBRATIONS = {
    ...
    mbappe: { label: "抱臂定格", detail: "冷静庆祝", style: "arms", duration: 2.15, color: "#243c94" },
    ...
    bellingham: { label: "伯纳乌展翼", detail: "张开双臂", style: "spread", duration: 2.3, color: "#b88a28" },
  ...
  const CELEBRATION_POSE_IMAGES = {
    mbappe: {},
    bellingham: {},
  };
  for (const [opponent, file] of [["mbappe", "mbappe"], ["bellingham", "bellingham"]]) {
    const defaultPose = new Image();
    defaultPose.src = `./assets/characters/celebrations/neymar-${file}-pose.png`;
    CELEBRATION_POSE_IMAGES[opponent].default = defaultPose;
    const barcaPose = new Image();
    barcaPose.src = `./assets/characters/celebrations/neymar-barca-${file}-pose.png`;
    CELEBRATION_POSE_IMAGES[opponent].barca = barcaPose;
  }
  const STAR_CARDS = [
  ...
  function hasChampionsLeagueTrophy() {
    return msnStage === 3 && journeyLevels.size >= LEVELS.length;
  }
  ...
  function applyFootballerReward(type) {
    ...
    if (type === "messi") {
      ...
      showFootballReward("⭐ 登贝莱庆祝！", "内马尔模仿登贝莱滑跪指天：获得 10 点积分与宝藏");
    } else if (type === "mbappe") {
      showFootballReward("🧊 姆巴佩冷静定格！", "内马尔抱臂模仿姆巴佩招牌庆祝：获得 10 点积分与宝藏");
  ...
  function finish(won) {
    ...
    const journeyComplete = journeyLevels.size >= LEVELS.length;
    const trophyWon = hasChampionsLeagueTrophy();
    const stampText = `${stampAdded ? "本次旅程新城市" : "本次旅程已到访"} · ${journeyLevels.size}/${LEVELS.length}`;
  ...
  function drawCelebrationEffects(style, displayHeight, elapsed, front) {
    ...
    ctx.lineCap = "round";
    ctx.lineJoin = "round";
    // Effects frame the celebration; the dedicated pose sprites handle the
    // actual body movement so no detached limbs or emoji are needed.
    ...
    if (front && style === "dance") {
      ...
      ctx.fillStyle = "rgba(255,255,255,.88)"; ctx.font = "1000 12px sans-serif"; ctx.textAlign = "center";
      ctx.fillText("✦", displayHeight * .24, -displayHeight * .71 + beat);
    } else if (front && style === "slide") {
  ...
  function celebrationPoseMix(progress) {
    const enter = Math.min(1, progress / .16);
    const exit = Math.min(1, (1 - progress) / .14);
    const amount = Math.min(enter, exit);
    return amount * amount * (3 - 2 * amount);
  }

  function drawCelebrationPoseFigure(image, displayHeight, facing, opacity, dark = false) {
    if (!image || !image.complete || image.naturalWidth <= 0 || opacity <= 0) return false;
    const poseHeight = displayHeight;
    const poseWidth = poseHeight * image.naturalWidth / image.naturalHeight;
    ctx.save();
    ctx.globalAlpha = opacity;
    ctx.scale(facing, 1);
    ctx.fillStyle = "rgba(18,27,42,.2)";
    ctx.beginPath(); ctx.ellipse(0, 1, poseWidth * .34, 4, 0, 0, Math.PI * 2); ctx.fill();
    if (dark) ctx.filter = "brightness(.2) saturate(.35)";
    ctx.drawImage(image, -poseWidth / 2, -poseHeight, poseWidth, poseHeight);
    ctx.restore();
    return true;
  }
  ...
  function drawPlayerFigurine(time) {
    ...
    if (celebrating) {
      ...
      else if (celebration.style === "arms") {
        const settle = Math.min(1, progress / .2);
        offsetY = (1 - settle) * 7 + Math.sin(elapsed * 7) * .5;
        scaleX = .94 + settle * .06; scaleY = .96 + settle * .04;
      }
      ...
      else if (celebration.style === "spread") {
        const step = Math.min(1, progress / .2);
        offsetX = player.facing * step * 7;
        offsetY = (1 - step) * 5 - Math.sin(progress * Math.PI) * 3;
        scaleX = .94 + step * .06; scaleY = .96 + step * .04;
      }
    ...
    const poseOutfit = msnStage === 3 ? "barca" : "default";
    const poseImage = celebrating ? CELEBRATION_POSE_IMAGES[player.celebrationType]?.[poseOutfit] : null;
    const poseReady = Boolean(poseImage && poseImage.complete && poseImage.naturalWidth > 0);
    const poseMix = poseReady ? celebrationPoseMix(progress) : 0;
    if (poseMix < 1) drawCollectibleFigure("neymar", 0, 0, displayHeight, player.facing, neymarOutfit, 1 - poseMix);
    const poseScale = player.celebrationType === "bellingham" ? 1.18 : 1.1;
    if (poseImage) drawCelebrationPoseFigure(poseImage, displayHeight * poseScale, player.facing, poseMix, neymarOutfit === "black");
