(() => {
  ...
  const NEYMAR_CELEBRATIONS = {
    ...
    mbappe: { label: "冷静定格", detail: "巨星姿态", style: "arms", duration: 1.12, color: "#243c94" },
  ...
  function resetLevel() {
    ...
    player.w = Math.round(52 * equippedScale); player.h = Math.round(52 * equippedScale); player.grounded = false; player.invincible = 0; player.facing = 1; player.powerShot = false; player.shotCooldown = 0; player.jumpCount = 0; player.growthLevel = 0; player.heroForm = "neymar"; player.celebrationTimer = 0; player.celebrationDuration = 0; player.celebrationType = "";
  ...
  function applyFootballerReward(type) {
    ...
    if (type === "messi") {
      ...
      showFootballReward("SIU！", "内马尔模仿C罗招牌庆祝：获得 10 点积分与宝藏");
    } else if (type === "dembele") {
      growPlayer();
    } else if (type === "vinicius") {
      player.heroForm = "black";
      showFootballReward("⚫ 维尼修斯暗影彩蛋！", "内马尔立即变为暗影公仔；MSN巴萨球衣状态下也会生效");
    } else {
  ...
  function updateMsnStage(score) {
    ...
    if (msnStage === 2) {
      ...
      rewardDescription.textContent = "1000分 + 6枚城市印章达成：换上巴萨战袍，继续寻找欧冠奖杯碎片";
    }
    rewardToast.classList.remove("hidden");
  ...
  function finish(won) {
    ...
    const rewardText = trophyWon
      ? "十城齐聚，欧冠大耳朵杯组装完成！"
      : (msnStage === 3
        ? `欧冠奖杯碎片 ${trophyPieceCount()}/3；集齐10城后举行巴萨MSN捧杯仪式`
        : (msnStage === 2
  ...
  function drawCelebrationEffects(style, displayHeight, elapsed, front) {
    const beat = Math.sin(elapsed * 13);
    ctx.lineCap = "round";
    ctx.lineJoin = "round";
    // Celebration effects stay behind or around the original figurine. We do
    // not draw replacement arms, legs or hands on top of the player artwork.
    if (!front && style === "meditate") {
      ctx.strokeStyle = "rgba(98,197,231,.68)"; ctx.lineWidth = 2.4;
      ctx.beginPath(); ctx.ellipse(0, -3, 29 + beat * 2, 7 + beat, 0, 0, Math.PI * 2); ctx.stroke();
      ctx.beginPath(); ctx.ellipse(0, -displayHeight * .48, 23 + beat * 2, 34 + beat * 2, 0, 0, Math.PI * 2); ctx.stroke();
    } else if (!front && style === "spread") {
      ctx.strokeStyle = "rgba(184,138,40,.58)"; ctx.lineWidth = 2.6;
      for (const side of [-1, 1]) {
        ctx.beginPath(); ctx.moveTo(side * 18, -displayHeight * .52); ctx.lineTo(side * 43, -displayHeight * .69); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(side * 20, -displayHeight * .43); ctx.lineTo(side * 49, -displayHeight * .43); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(side * 18, -displayHeight * .34); ctx.lineTo(side * 43, -displayHeight * .18); ctx.stroke();
      }
    } else if (!front && style === "point") {
      ctx.strokeStyle = "rgba(201,47,79,.55)"; ctx.lineWidth = 3.2;
      ctx.beginPath(); ctx.moveTo(-44, -2); ctx.lineTo(-19, -2); ctx.moveTo(-50, 6); ctx.lineTo(-23, 6); ctx.stroke();
      ctx.fillStyle = "rgba(244,202,72,.9)"; ctx.font = "1000 14px sans-serif"; ctx.textAlign = "center"; ctx.fillText("★", 0, -displayHeight - 5);
    } else if (!front && style === "siu") {
      ctx.strokeStyle = "rgba(181,29,44,.52)"; ctx.lineWidth = 2.6;
      for (const side of [-1, 1]) {
        ctx.beginPath(); ctx.moveTo(side * 17, -displayHeight * .2); ctx.lineTo(side * 34, -displayHeight * .06); ctx.stroke();
      }
      ctx.globalAlpha = .16; ctx.fillStyle = "#b51d2c"; ctx.font = "1000 29px ui-rounded, sans-serif"; ctx.textAlign = "center"; ctx.fillText("SIU", 0, -displayHeight * .47); ctx.globalAlpha = 1;
    } else if (!front && style === "arms") {
      const glow = ctx.createRadialGradient(0, -displayHeight * .5, 4, 0, -displayHeight * .5, 40);
      glow.addColorStop(0, "rgba(97,139,232,.34)"); glow.addColorStop(1, "rgba(36,60,148,0)");
      ctx.fillStyle = glow; ctx.beginPath(); ctx.arc(0, -displayHeight * .5, 40, 0, Math.PI * 2); ctx.fill();
    } else if (!front && style === "slide") {
      ctx.strokeStyle = "rgba(39,59,112,.5)"; ctx.lineWidth = 3.4;
      ctx.beginPath(); ctx.moveTo(-43, -4); ctx.lineTo(-17, -4); ctx.moveTo(-49, 4); ctx.lineTo(-21, 4); ctx.stroke();
    }

    if (front && style === "dance") {
      ctx.fillStyle = "#f4ca48"; ctx.font = "1000 15px sans-serif"; ctx.textAlign = "center";
      ctx.fillText("♪", -30, -displayHeight * .62 + beat * 2); ctx.fillText("♫", 30, -displayHeight * .77 - beat * 2);
    } else if (front && style === "arms") {
      ctx.fillStyle = "rgba(255,255,255,.82)"; ctx.font = "1000 11px sans-serif"; ctx.textAlign = "center";
      ctx.fillText("✦", 25, -displayHeight * .7 + beat);
    } else if (front && style === "slide") {
      ctx.fillStyle = "rgba(244,202,72,.88)"; ctx.font = "1000 11px sans-serif"; ctx.textAlign = "center";
      ctx.fillText("★", 25, -displayHeight * .7 + beat);
    }
  }
  ...
  function drawPlayerFigurine(time) {
    ...
    const neymarOutfit = activeForm === "black" ? "black" : (msnStage === 3 ? "barca" : activeForm);
    ...
    if (celebrating) {
      if (celebration.style === "siu") { offsetY = -Math.sin(progress * Math.PI) * 20; rotation = Math.sin(progress * Math.PI * 2) * .1; }
      else if (celebration.style === "meditate") { offsetY = 2 + Math.sin(elapsed * 7) * 1.2; scaleY = .9; scaleX = 1.03; }
      else if (celebration.style === "arms") { scaleX = 1.025; offsetY = Math.sin(elapsed * 8) * 1.2; }
      else if (celebration.style === "point") { offsetX = player.facing * progress * 10; offsetY = progress * 3; rotation = player.facing * -.035; scaleY = .95; }
      else if (celebration.style === "dance") { offsetX = Math.sin(elapsed * 13) * 7; rotation = Math.sin(elapsed * 13) * .12; }
      else if (celebration.style === "spread") { offsetY = -3 - Math.sin(progress * Math.PI) * 3; scaleX = 1.03; }
      else if (celebration.style === "slide") { offsetX = player.facing * progress * 11; offsetY = progress * 3; rotation = player.facing * -.025; scaleY = .95; }
    }
    ...
    ctx.save(); ctx.translate(centerX + offsetX, bottomY + offsetY); ctx.rotate(rotation); ctx.scale(scaleX, scaleY);
    if (celebrating) drawCelebrationEffects(celebration.style, displayHeight, elapsed, false);
    drawCollectibleFigure("neymar", 0, 0, displayHeight, player.facing, neymarOutfit);
    if (celebrating) drawCelebrationEffects(celebration.style, displayHeight, elapsed, true);
    ctx.restore();
    const trophyWon = hasChampionsLeagueTrophy();
    if (trophyWon && !celebrating) drawChampionsLeagueTrophy(centerX, bottomY - displayHeight * .31, time);
    ctx.save();
    const teamLabel = celebrating ? `⚽ 内马尔 · ${celebration.label}` : (trophyWon ? "🏆 巴萨MSN合捧欧冠大耳朵杯" : (msnStage === 3 ? "🔵🔴 巴萨MSN 3/3" : (msnStage === 2 ? "MSN 2/3 · 梅西加入" : "内马尔 · 10")));
    const labelWidth = celebrating ? 142 : (trophyWon ? 174 : (msnStage === 1 ? 78 : 124));
  ...
  function drawChampionsLeagueTrophy(x, y, time) {
    const sway = Math.sin(time * 3) * .45;
    ctx.save(); ctx.translate(x, y + sway); ctx.scale(.72, .72);

    // Barcelona-blue and garnet ribbons sit behind the two iconic handles.
    ctx.lineCap = "round";
    ctx.strokeStyle = "#2451a4"; ctx.lineWidth = 4.2;
    ctx.beginPath(); ctx.moveTo(-17, -14); ctx.quadraticCurveTo(-25, -3, -21, 15); ctx.stroke();
    ctx.strokeStyle = "#a82346";
    ctx.beginPath(); ctx.moveTo(17, -14); ctx.quadraticCurveTo(25, -3, 21, 15); ctx.stroke();

    const silver = ctx.createLinearGradient(-18, -24, 18, 25);
    silver.addColorStop(0, "#ffffff"); silver.addColorStop(.2, "#cbd7e5");
    silver.addColorStop(.46, "#f8fbff"); silver.addColorStop(.7, "#8999ad"); silver.addColorStop(1, "#e7eef7");
    ctx.shadowColor = "rgba(196,224,255,.78)"; ctx.shadowBlur = 9;

    // Large open handles create the unmistakable European Cup silhouette.
    ctx.strokeStyle = "#53657a"; ctx.lineWidth = 5.5;
    ctx.beginPath(); ctx.moveTo(-12, -16); ctx.bezierCurveTo(-34, -23, -34, 8, -12, 5); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(12, -16); ctx.bezierCurveTo(34, -23, 34, 8, 12, 5); ctx.stroke();
    ctx.strokeStyle = silver; ctx.lineWidth = 3.2;
    ctx.beginPath(); ctx.moveTo(-12, -16); ctx.bezierCurveTo(-31, -20, -31, 5, -12, 3); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(12, -16); ctx.bezierCurveTo(31, -20, 31, 5, 12, 3); ctx.stroke();

    // Wide silver bowl, narrow stem and layered base.
    ctx.fillStyle = silver; ctx.strokeStyle = "#53657a"; ctx.lineWidth = 1.7;
    ctx.beginPath();
    ctx.moveTo(-13, -18); ctx.quadraticCurveTo(-13, -3, -7, 6);
    ctx.quadraticCurveTo(-4, 10, -3, 16); ctx.lineTo(3, 16);
    ctx.quadraticCurveTo(4, 10, 7, 6); ctx.quadraticCurveTo(13, -3, 13, -18);
    ctx.closePath(); ctx.fill(); ctx.stroke();
    ctx.beginPath(); ctx.ellipse(0, -18, 13.5, 4.2, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
    ctx.shadowBlur = 0;
    ctx.fillStyle = "rgba(62,78,97,.5)"; ctx.beginPath(); ctx.ellipse(0, -17.7, 9.4, 2.4, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = "#eef4fb"; ctx.fillRect(-3.3, 14, 6.6, 7);
    drawRoundedRect(-11, 19, 22, 4.6, 1.8, silver, "#53657a", 1.4);
    drawRoundedRect(-14, 23, 28, 5.2, 1.8, silver, "#53657a", 1.4);

    // Three pairs of small hands meet on the cup rather than using long arm bars.
    const hands = [
      { color: "#d89a72", x: -11.5, y: 1 }, { color: "#d89a72", x: -7.5, y: 5 },
      { color: "#b9784d", x: -2.5, y: 10 }, { color: "#b9784d", x: 2.5, y: 10 },
      { color: "#c17a50", x: 7.5, y: 5 }, { color: "#c17a50", x: 11.5, y: 1 },
    ];
    for (const hand of hands) {
      ctx.fillStyle = hand.color; ctx.strokeStyle = "#5c3b2f"; ctx.lineWidth = 1;
      ctx.beginPath(); ctx.arc(hand.x, hand.y, 2.25, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
    }

    const glint = .7 + Math.sin(time * 4) * .25;
    ctx.globalAlpha = glint; ctx.fillStyle = "#ffffff";
    ctx.beginPath(); ctx.moveTo(-8, -12); ctx.lineTo(-6.8, -9.5); ctx.lineTo(-4.3, -8.3); ctx.lineTo(-6.8, -7.1); ctx.lineTo(-8, -4.6); ctx.lineTo(-9.2, -7.1); ctx.lineTo(-11.7, -8.3); ctx.lineTo(-9.2, -9.5); ctx.closePath(); ctx.fill();
    ctx.globalAlpha = 1;
    ctx.restore();
  }
  ...
  function drawHud() {
    ...
    ctx.fillStyle = trophyWon ? "#45658f" : "#2857a1"; ctx.font = "900 14px ui-rounded, sans-serif";
    ctx.fillText(trophyWon ? "🏆 欧冠大耳朵杯" : (msnStage === 3 ? `🧩 欧冠杯 ${trophyPieceCount()}/3` : `🗺 印章 ${stampCount}/10`), trophyBoxX + trophyBoxW / 2, 41);
