(() => {
  ...
  function saveMsnClassicCards() {
    try { localStorage.setItem("hat-adventure-msn-classic-cards", JSON.stringify([...ownedMsnCards])); } catch (_) {}
  }
  ...
  function drawMsnClassicCard() {
    ...
    if (!missing.length) return { card: null, setCompleted: true, alreadyComplete: true };
    const card = missing[Math.floor(Math.random() * missing.length)];
    ownedMsnCards.add(card.id);
    saveMsnClassicCards();
    const setCompleted = ownedMsnCards.size === MSN_CLASSIC_CARDS.length;
    if (setCompleted) addWallet(500);
    return { card, setCompleted, alreadyComplete: false };
  }
  ...
  function renderJourneyAssets() {
    ...
    journeyAssetGrid.innerHTML = `
      ...
      <section class="victory-photo ${photoUnlocked ? "" : "locked"}"><img src="./assets/msn-classics/msn-ending-photo.png" alt="MSN冠军合影"><div><strong>${photoUnlocked ? "MSN冠军合影" : "冠军合影待解锁"}</strong><small>${photoUnlocked ? "首次通关第10关后永久收藏" : "击败第10关终极Boss后获得"}</small></div></section>
  ...
  function finish(won) {
    ...
    if (finalVictory) {
      const cardNotice = drawResult?.card
        ? `已抽到一张未收藏的MSN经典时刻卡${drawResult.setCompleted ? "，四卡全集已完成并加赠500宝藏" : ""}`
        : "MSN经典时刻四卡全集已收藏";
      messageText.textContent = `因凡蒂诺已被击败，大耳朵杯与冠军合影已收藏；${cardNotice}。全部奖励请到资产页查看。`;
    } else if (won) {
